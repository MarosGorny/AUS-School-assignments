...
namespace structures
{
	...
	inline int SortedSequenceTable<K, T>::indexOfKey(K key, int indexStart, int indexEnd, bool& found)
	{
		...
		K keyMid = this->list_->at(midIndex)->getKey();
		...
		if (keyMid == key) {
			...
			if (indexStart == indexEnd) {
				...
				if (keyMid < key) {
					int newIndexStart = midIndex + 1;
					return this->indexOfKey(key, newIndexStart, indexEnd, found);
				}
				else {
					int newIndexEnd = midIndex;
					return this->indexOfKey(key, indexStart, newIndexEnd, found);
				}
			}
		}
	}
