...
namespace structures
{
	...
	{
		BSTTreeNode* newNode = new BSTTreeNode(new TableItem<K, T>(key, data));
	...
	inline T BinarySearchTree<K, T>::remove(const K& key)
	{
		...
		//sposobom takym, ze budem volat metodu ExtractNode
	}
	...
	{
		for (TableItem<K, T>* item : *this->binaryTree_) { //ale moze byt aj iba this, bez ->binaryTree
			delete item;
		}
